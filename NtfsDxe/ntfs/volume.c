...
{
	return (ntfs_volume *) ntfs_calloc(sizeof(ntfs_volume));
}
...
static int ntfs_mft_load(ntfs_volume *vol)
{
	...
	if (l <= 0 || l > 0x40000) {
		ntfs_log_error("$MFT/$ATTR_LIST invalid length (%l).\n",
			       (long long)l);
		goto io_error_exit;
	}
	vol->mft_ni->attr_list_size = l;
	vol->mft_ni->attr_list = (u8 *) ntfs_malloc(l);
	...
	if (l != vol->mft_ni->attr_list_size) {
		ntfs_log_error("Partial read of $MFT/$ATTR_LIST (%l != "
			       "%u).\n", (long long)l,
...
mft_has_no_attr_list:
	...
	if (highest_vcn && highest_vcn != last_vcn - 1) {
		...
		ntfs_log_error("highest_vcn = 0x%lx, last_vcn - 1 = 0x%lx\n",
	...
	set_nino_flag(vol->mft_ni, NI_KnownSize);
		...
		ntfs_mount_flags flags)
{
	...
	ntfs_log_debug("mft_zone_pos = 0x%lx\n", (long long)vol->mft_zone_pos);
	...
	ntfs_log_debug("mft_zone_start = 0x%lx\n", (long long)vol->mft_zone_start);
	...
	ntfs_log_debug("mft_zone_end = 0x%lx\n", (long long)vol->mft_zone_end);
	...
	ntfs_log_debug("data1_zone_pos = %l\n", (long long)vol->data1_zone_pos);
	vol->data2_zone_pos = 0;
	ntfs_log_debug("data2_zone_pos = %l\n", (long long)vol->data2_zone_pos);
...
static ntfs_inode *ntfs_hiberfile_open(ntfs_volume *vol)
{
	...
	if (!ni_hibr) {
		ntfs_log_debug("Couldn't open inode %l.\n", (long long)inode);
		goto out;
	}
out:
...
ntfs_volume *ntfs_device_mount(struct ntfs_device *dev, ntfs_mount_flags flags)
{
	...
	if (vol->lcnbmp_na->data_size > vol->lcnbmp_na->allocated_size) {
		ntfs_log_error("Corrupt cluster map size (%l > %l)\n",
...
ntfs_volume *ntfs_mount(const char *name ,
		ntfs_mount_flags flags, void *ptr)
{
	...
	dev = ntfs_device_alloc(name, 0, &ntfs_device_default_io_ops, ptr);
	if (!dev)
		return NULL;
